...
struct memop_args
{
    ...
    extent_list;               /* List of extent base addrs. */
...
static void increase_reservation(struct memop_args *a)
{
    ...
    printk(KERN_WARNING "Increasing reservation...");
...
static void populate_physmap(struct memop_args *a)
{
    struct page_info *page=NULL;
     // struct page_info *temp_page=NULL;
    unsigned int i, j;
    xen_pfn_t gpfn, mfn;
    struct domain *d = a->domain;
    int contiguity_test = 1;
    ...
    if (a->extent_order > (a->memflags & MEMF_populate_on_demand ? MAX_ORDER : max_order(current->domain)))
        return;
    ...
    for (i = a->nr_done; i < a->nr_extents; i++)
    {
        ...
        {
            //printk(KERN_WARNING "yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy 2\n");
            goto out;
        }

        if (a->memflags & MEMF_populate_on_demand)
        {
            //printk(KERN_WARNING "Did populate on demand \n");
        ...
        {    /* No need to touch direct mapping as they are not used in page tables ...pyuhala */
                ...
                for (j = 0; j < (1U << a->extent_order); j++, mfn++)
                {
                    if (!mfn_valid(mfn))
                    {
                        gdprintk(XENLOG_INFO, "Invalid mfn %#" PRI_xen_pfn "\n",
                                 mfn);
                        // printk(KERN_WARNING "yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy 4\n");
                        goto out;
                    }
            ...
            else
            {
               ...
               //printk(KERN_WARNING " Pool size : %lu \n", free_pages);
                ...
                if (unlikely(!page))
                {
                    //printk(KERN_WARNING "A page was unlikely \n");
            ...
            {
                // printk(KERN_WARNING "Entered not paging mode translate...\n");
    ...
    // printk(KERN_WARNING "Count: %u \n",count);
    // attrib.domain=a->domain;
    // attrib.base=base;
    // printk(KERN_WARNING " Count : %d \n", count);
    /*if (contiguity_test)
    {

        printk(KERN_WARNING "Domain: %d CONTIGUOUS MEMORY : Base = %lu \n", d->domain_id, base);
    }
    else
    {
        printk(KERN_WARNING "Domain: %d NON CONTIGUOUS MEMORY \n", d->domain_id);
    }*/

    //printk(KERN_WARNING "Total frames: %u ",count);

out:
    a->nr_done = i;
    //printk(KERN_WARNING "yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy %d \n",i);
...
int guest_remove_page(struct domain *d, unsigned long gmfn)
{
    ...
    guest_physmap_remove_page(d, gmfn, mfn, 0);
    // printk(KERN_WARNING "Removing pages");
    put_page(page);
    put_gfn(d, gmfn);
...
static void decrease_reservation(struct memop_args *a)
{
    ...
    // printk(KERN_WARNING "Decreasing reservation...");
    ...
    for (i = a->nr_done; i < a->nr_extents; i++)
    {
        ...
        if (is_hvm_domain(a->domain) && p2m_pod_decrease_reservation(a->domain, gmfn, a->extent_order))
            continue;
...
static long memory_exchange(XEN_GUEST_HANDLE_PARAM(xen_memory_exchange_t) arg)
{
    ...
    memflags |= MEMF_bits(domain_clamp_alloc_bitsize(
        d,
        XENMEMF_get_address_bits(exch.out.mem_flags) ?: (BITS_PER_LONG + PAGE_SHIFT)));
...
long do_memory_op(unsigned long cmd, XEN_GUEST_HANDLE_PARAM(void) arg)
{
    ...
    case XENMEM_populate_physmap:
        ...
        if (op == XENMEM_populate_physmap && (reservation.mem_flags & XENMEMF_populate_on_demand))
        ...
        default: /* XENMEM_populate_physmap */
                 // printk(KERN_WARNING "yyyyyyyyyyyyyyyyyyyyyyyyyyyy %s \n", __func__);
