...
#define SUPERPAGE_BATCH_SIZE 131072

#define SUPERPAGE_2MB_SHIFT 17
...
static int start_info_x86_32(struct xc_dom_image *dom)
{
    ...
    xen_pfn_t shinfo =
        xc_dom_feature_translated(dom) ? dom->shared_info_pfn : dom->shared_info_mfn;
...
static int meminit_hvm(struct xc_dom_image *dom)
{
    ...
    for (i = 0; i < nr_vmemranges; i++)
    {
        ...
        while (super_pages)
        {
            ...
            if (rc < 0)
                return rc;
            //fprintf(stderr,"Reached here, rc positive");
            /* Expand the returned mfns into the p2m array. */
            pfn = pfn_base_idx;
            for (j = 0; j < rc; j++)
            {
