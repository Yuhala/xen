              ...
              libxl__domain_build_state *state)
{
   ...
   // device_model=false;
     ...
     */
    fprintf(stderr,"HVM rdm mem boundary: %" PRIu64 ".\n",info->u.hvm.rdm_mem_boundary_memkb >> 10);
    fprintf(stderr,"Info target mem %" PRIu64 ".\n",info->target_memkb >> 10);
    fprintf(stderr,"Info max mem %" PRIu64 ".\n",info->max_memkb >> 10);
    fprintf(stderr,"Info video mem %" PRIu64 ".\n",info->video_memkb >> 10);
    fprintf(stderr,"Info mmio hole %" PRIu64 ".\n",info->u.hvm.mmio_hole_memkb);
    ...
    fprintf(stderr,"Low mem end:%" PRIu64 ".\n",dom->lowmem_end);
    fprintf(stderr,"High mem end: %" PRIu64 ".\n",dom->highmem_end);
    fprintf(stderr,"MMIO start: %" PRIu64 ".\n",dom->mmio_start );
    fprintf(stderr,"VGA hole size: %" PRIu64 ".\n",dom->vga_hole_size );

    //
   /* dom->device_model =0; //Remove device model 
    dom->vga_hole_size=0; //with a zero vga hole, we won't skip thereby improving chances of contiguous memory
    dom->mmio_size=0;
    dom->mmio_start=highmem_end; */ //remove mmio hole by setting mmio size to zero and start to highmem end

   // fprintf(stderr,"HVM rdm mem boundary %" PRIu64 ".\n",info->u.hvm.rdm_mem_boundary_memkb);
